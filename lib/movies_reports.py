...
def print_group(group_number, group_data):
    ...
    print_status(group_data)


def print_all_movies(movie_data):
    ...
    for group in groups:
        ...
        print_group(group, group_data)
        print("\n")

    print_status(movie_data)


def print_search_results(pattern, search_results):
    ...
    for search_result in search_results:
        print(RECORD_FORMAT.format(search_result["watched"], search_result["id"], search_result["group_id"],
                                   search_result["movie_id"], search_result["movie_name"], search_result["duration"],
...
def print_status(data):
    ...
    time_left = format_duration(sum(list(map(lambda x: x[1], list(filter(lambda x: x[0] == 0, status_tuple_array))))))
